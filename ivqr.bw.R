...
ivqr.bw <- function(p, va, Xt, lX, Lagphi, b.init) {
  r <- 4
  if (!is.matrix(va)) va <- matrix(va,ncol=1)
  # Defaults, initialization
  n <- length(va) #number of observations, a.k.a. sample size
  d <- length(b.init) #number of regressors
  ...
  # Plug-in bandwidth as function of distributional parameters, for different possible parametric distributions.
	...
	Uhat <- va-Xt%*%b.init[1:(length(b.init)-1)]-b.init[length(b.init)]*(Lagphi-lX%*%b.init[1:(length(b.init)-1)])
