...
require(stringr)
#Load US dataset
USdata <- read.csv("PFQR/DATA/US/USdata.csv")
print(head(USdata))
#Convert 3 digit isic code to 2 digit isic and take natural logs
# USdata <- transmute(US_panel, id=id, year=year, lny=log(Y), lnva=log(VA), lnk=log(K2), lnl=log(L), lnm=log(M), adv=adv, rd=rd, advB=advB, rdB=rdB, naics3=as.character(naics3), naics2=as.character(str_extract(as.character(naics3), "^.{2}")))
# USdata <- transmute(US_panel, id=id, year=year, lny=lny, lnva=lnva, lnk=lnk1, lnl=lnl, lnm=lnm, adv=adv, rd=rd, advB=advB, rdB=rdB, naics2=naics2)
...
for (j in 1:tau_n){
  ...
  soln <- QACF_Boot(tau=tauvec[j], idvar=US$id, timevar=US$year, Y=US$lnva, K=US$lnk1, L=US$lnl, proxy=US$lnm, dZ=dZ, binit=NULL, R=R, XC=cbind(US$adv, US$rd), XB=cbind(US$advB, US$rdB))
...
for (j in 1:tau_n){
  print(sprintf("tau=%s", tauvec[j]))
  soln <- QLP_Boot(tau=tauvec[j], idvar=US$id, timevar=US$year, Y=US$lny, K=US$lnk1, L=US$lnl, proxy=US$lnm, dZ=dZLP, binit=NULL, R=R, XC=cbind(US$adv, US$rd), XB=cbind(US$advB, US$rdB))
  #Bootstrapped Estimates
