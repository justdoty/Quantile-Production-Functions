...
#########################
gmmq <- function(tau, mX, mlX, mZ, fitphi, fitlagphi, Lambda=function(theta, mX, mlX, fitphi, fitlagphi, tau){fitphi-mX%*%theta[1:(ncol(mX))]-theta[length(theta)]*(fitlagphi-mlX%*%theta[1:(ncol(mX))])}, 
  Lambda.derivative=function(theta, mX, mlX, fitphi, fitlagphi, tau){fitphi-mX%*%theta[1:(ncol(mX))]-theta[length(theta)]*(fitlagphi-mlX%*%theta[1:(ncol(mX))])}, h=0, VERBOSE=FALSE, theta.init=0) {
  #Sample Size
  dZ <- ncol(mZ)
  n <- dim(Y)[1]
  ...
  if (!is.null(Lambda.derivative)) {
    jac.fn <- function(theta,h) {
      L <- Lambda(theta, mX=mX, mlX=mlX, fitphi=fitphi, fitlagphi=fitlagphi, tau=tau)
      Lp <- Lambda.derivative(theta, mX=mX, mlX=mlX, fitphi=fitphi, fitlagphi=fitlagphi, tau=tau)
