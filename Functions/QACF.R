...
###################################################################################
###################################################################################
QACF <- function(tau, idvar, timevar, Y, K, L, proxy, dZ, binit=NULL){
  seed <- 123456
  ...
  #Here ind denotes the index that does tells finalQACF not resample the original data
  res <- finalQACF(tau=tau, data=data, binit=binit, seed=seed)
  #"True" parameters
  betahat <- res$beta
  ACFhat <- res$ACFhat
  return(list(betahat=betahat, ACFhat=ACFhat))
}
###########################################################################
###########################################################################
...
###########################################################################
###########################################################################
finalQACF <- function(tau, h, data, binit, seed){
  set.seed(seed)
  #######################################################################
  #First Stage of ACF
  #######################################################################
