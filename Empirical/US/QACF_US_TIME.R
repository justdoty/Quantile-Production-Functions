...
for (t in 1:length(split)){
  ...
  soln <- QACF(tau=tau_t, idvar=US$id, timevar=US$year, Y=US$lnva, K=US$lnk, L=US$lnl, proxy=US$lnm, dZ=dZ, binit=NULL)
  QACFbetahat[,t] <- soln$betahat
  ACFhat[,t] <- soln$ACFhat
}
filename <- paste("PFQR/DATA/US/QACF/QACF_Environments/QACFT_US_Q", id, ".RData", sep="")
save(QACFbetahat, ACFhat, file=filename)


#HPC Job Submissions for batches: qsub -t 1:length(tau_t) QACF_US_TIME.job
#Here length(tau_t)=4
