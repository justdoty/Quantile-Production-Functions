...
sumind <- ISIC
summary <- do.call(cbind, lapply(sumind, function(x) summary_table(dplyr::filter(CHLdata, str_detect(isic3, x)), format)))
...
isicsize <- group_by(CHLdata, isic3) %>% summarise(Firms=length(unique(id)), Total=n())
...
for (i in 1:length(tau)){
	load(sprintf("/Users/justindoty/Documents/Research/Dissertation/Production_QR_Proxy/Code/Empirical/Chile/Environments/QLP_CHL_Q%s.RData", i))
...
for (p in 1:length(ISIC)){
  ...
  ISIC_plots <- ggdraw() + draw_label(paste("ISIC", ISIC_relabel[p], sep=""), fontface="plain")
  Coef_Plot <- plot_grid(ISIC_plots, L_plot[[p]], K_plot[[p]], ncol=1)
  save_plot(paste("/Users/justindoty/Documents/Research/Dissertation/Production_QR_Proxy/Code/Empirical/Chile/Plots/Coef_Plot_ISIC_", ISIC_relabel[p], ".png", sep=""), Coef_Plot, base_height=10, base_width=5)
}
