...
for (isic in 1:length(industries)){
	...
	lm.soln[isic,] <-lm(CHL$lnva~CHL$lnk+CHL$lnl)$coef[-1]
	for (q in 1:length(tau)){
		qr.soln[,,isic][q,] <- rq(CHL$lnva~CHL$lnk+CHL$lnl, tau=tau[q])$coef[2:3]
	}
}
save(lm.soln, qr.soln, file='/Users/justindoty/Documents/Research/Dissertation/Production_QR_Proxy/Code/Empirical/Chile/Environments/CHL_QR_OLS.Rdata')
