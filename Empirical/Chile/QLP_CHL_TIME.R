...
for (t in 1:length(split)){
  ...
  soln <- tryCatch(QLP(tau=tau_t, va=CHL$lnva, state=CHL$lnk, free=CHL$lnl, proxy=CHL$lnm, id=CHL$id, time=CHL$year, h=h, b.init=NULL, R=R))
  results_T[,,t] <- soln[[1]]
  true.beta_T[,t] <- soln[[2]]
}
filename <- paste("QLPT_CHL_Q", id, ".RData", sep="")
