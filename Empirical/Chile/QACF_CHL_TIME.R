...
for (t in 1:length(split)){
  ...
  soln <- QACF(tau=tau_t, idvar=CHL$id, timevar=CHL$year, Y=CHL$lnva, K=CHL$lnk, L=CHL$lnl, proxy=CHL$lnm, dZ=dZ, binit=NULL)
  QACFbetahat[,t] <- soln$betahat
  ACFhat[,t] <- soln$ACFhat
}
filename <- paste("PFQR/DATA/CHL/QACF/QACF_Environments/QACFT_CHL_Q", id, ".RData", sep="")
